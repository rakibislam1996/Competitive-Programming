...
vll prime,pFactor;
ui status[(MX>>6)+2];
template<typename T>inline void bitsieve(T N)
{
    T i,sq=sqrt(N)+2;
    prime.pb(2);
    for(i=3; i<=N; i+=2)
...
template<typename T>inline void sieve(T N)
{
    T i,sq=sqrt(N)+2;
    status[0]=status[1]=1;
    prime.pb(2);/*for(i=4;i<=N;i+=2)status[i]=1;*/for(i=3; i<=N; i+=2)
        if(!(status[i]))
        {
            prime.pb(i);
            if(i<=sq)
...
template<typename T>inline T noOfprime(T n,T p)
{
    T res=0;
    while(n>=p)
        res+=n/p,n/=p;
...
template<typename T>inline void factor(T N)
{
    while(!(N&1))
    {
        pFactor.pb(2);
...
template<typename T>inline T totientws(T N)
{
    ll ans=N;
    if(!(N&1))
    {
...
template<typename T>inline T bigMul(T a,T b,T m)
{
    if(b==0)
        return 0;
    ull x=bigMul(a,b>>1,m);
    x=((x%m)+(x%m))%m;
    if(b&1)
        x=(x+(a%m))%m;
    return x;
...
template<typename T>inline T bigMod(T a,T b,T m)
{
    if(b==0)
        return 1;
    ull x=bigMod(a,b>>1,m);
    x=((x%m)*(x%m))%m;
    if(b&1)
        x=(x*(a%m))%m;
    return x;
}
template<typename T>inline T bigmodmul(T a,T b,T m)
{
    if(b==0)
        return 1;
    ull x=bigmodmul(a,b>>1,m);
    x=bigMul(x,x,ull(m));
    if(b&1)
        x=bigMul(x,ull(a),ull(m));
    return x;
}
template<typename T>inline T negMod(T n,T m)
{
    return (n%m+m)%m;
}
...
template<typename T>inline void in2(T &a,T &b)
{
    in(a);
    in(b);
}
template<typename T>inline void in3(T &a,T &b,T &c)
{
    in(a);
    in(b);
    in(c);
...
template<typename T>inline T division(string str,T m)
{
    T quitient=0,remainder=0,i;
    for(i=0; str[i]; i++)
    {
        quitient=quitient*10+(remainder*10+(str[i]-'0'))/m;
        remainder=(remainder*10+(str[i]-'0'))%m;
    }
    return remainder;
}
...
template<typename T>inline string ItoS(T n)
{
    SS ss;
    ss << n;
    return ss.str();
...
template<typename T>inline T BtoD(T x)
{
    T two=1,intn=0;
    while(x)
    {
...
{
    ch=toupper(ch);
    return(ch=='A'||ch=='U'||ch=='I'||ch=='O'||ch=='E');
    return false;
}
