...
#define Auto(i,v)       __typeof(v.begin())i
#define E               "\n"
#define e               putchar('\n')
...
#define sz 100005
ll arr[sz];
ll block[400],blocksz;

void build(ll idx,ll val)
{
    ll id = idx/blocksz;
...
ll query(ll l,ll r)
{
    ...
    if(lid==rid)
    {
        if(l%blocksz==0 && r%blocksz==blocksz-1)  /// Full block
            return block[lid];
        else
        {
            ll ret = -inf;
    ...
    else
    {
        ...
        For(i,lid+1,rid-1)                  /// Middle blocks
            m1 = max(m1,block[i]);

        ll stIdxL = l%blocksz;        /// start index for left most block

        For(i,stIdxL,blocksz-1)                /// Left most block
            m2 = max(m2,arr[i+ lid*blocksz]);


        ll enIdxR = r%blocksz;        /// ending index for right most block

        For(i,0,enIdxR)                /// right most block
            m3 = max(m3,arr[i+ rid*blocksz]);
...
int main()
{
    ...
    TC(T)
    {
        ...
        blocksz = ceil(sqrt(n));   /// To approach with 1 based index every calc will be with ceil()
