...
bool second(pair<int,int> x, pair<int,int> y)
{
    return x.S<y.S;
}
bool last_digit(int x, int y)
{
    return x%10<y%10;
}
...
int main()
{
    ...
    vector<pair<int,int>> v1={{4,5},{1,7},{9,3},{2,1},{9,4},{8,6}};
    ...
    vector<int> v = { 3, 0, 7, 10, 2, 4, 8, 17, 5, 16, 19};
    ...
    vector<int> v2 = { 19, 18, 17, 16, 12, 14, 18, 14, 15, 16, 20};

    ///nth_element(v2.begin(),v2.begin()+4,v2.end());  /// order is not maintained
    nth_element(v2.begin(),v2.begin()+4,v2.end(),greater<int>());  /// order is not maintained
    printStl(v2);
    e;

    int arr[] = {19, 63, 77, 16, 12, 34, 18, 13, 51, 29};
    int arr1[] = {-19, -63, -77, -16, -12, -34, -18, -13, -51, -29};

    int j;
    sort(arr,arr+10,last_digit);
    For(j,0,9)
        cout<<arr[j]<<" ";
    e;
    partial_sort(arr,arr+5,arr+10,last_digit);
    For(j,0,9)
        cout<<arr[j]<<" ";
    e;
