...
template<typename T>inline T SODofDivisors(T N)
{
    T sz=prime.size(),sod=1;
    for(T i=0; i<sz&&prime[i]*prime[i]<=N; i++)
    {
        T p=prime[i],term=prime[i],sum=1;
        if(!(N%p))
        {
            while(!(N%p))
            {
                ...
                //sum += ((term-1)/(p-1));
                term = (term * p) % mod;
                sum = ( sum % mod + ( (term-1) % mod * inverseMod(p-1,mod)) % mod ) % mod;
                N/=p;
            }
            //sod *= sum;
            sod = (sod * sum) % mod;
    ...
    if(N>1)
        sod = (sod * ( 1+( ((N*N)%mod-1) * inverseMod(N-1,mod) ) )) % mod;
        //sod *= (1+((N*N-1)/(N-1)));
    return sod;
}
