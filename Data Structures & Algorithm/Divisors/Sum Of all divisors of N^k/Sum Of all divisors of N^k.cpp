...
ulli SOD(ulli N,ulli M)
{
    ...
    for(long i=0; prime[i]*prime[i]<=N; i++)
    {
        ...
        if(!(N%temp))
        {
            ...
            count = (M*count)+1;
            ulli lob = (bigMod(temp,count,MOD)-1)%MOD;
            ulli hor = bigMod(temp-1,MOD-2,MOD);
            ulli s = (( lob%MOD) * (hor%MOD )) % MOD;
    ...
    if(N>1)
    {
        ...
        {
            ulli lob = (bigMod(N,M+1,MOD)-1)%MOD;
            ulli hor = bigMod(N-1,MOD-2,MOD);
            ulli s = (( lob%MOD) * (hor%MOD )) % MOD;
            ans=((ans%MOD)*(s%MOD))%MOD;
        }
    }
