...
void update(int node,int st,int en,int i,int j,lli val)   /// O(logN)
{
    if(i>en || j<st)
        return;
    if(st>=i && en<=j)
    ...
    tree[node].sum = tree[lft].sum + tree[rgt].sum + (en-st+1)*tree[node].prop;

}
int query(int node,int st,int en,int i,int j,int carry=0)   /// O(logN)
{
    if(i>en || j<st)
        return 0;
    if(st>=i && en<=j)
    ...
    int p1 = query(lft,st,md,i,j,carry+tree[node].prop);
    int p2 = query(rgt,md+1,en,i,j,carry+tree[node].prop);
