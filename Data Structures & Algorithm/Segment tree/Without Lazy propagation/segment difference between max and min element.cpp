...
void build(int node,int st,int en)   /// O(N logN)
{
    if(st==en)
    {
        mx[node] = arr[st];
    ...
    int md = st + ((en-st)>>1);
    int lft = node<<1;      /// 2*node
    int rgt = (node<<1)+1;

    build(lft,st,md);
    build(rgt,md+1,en);

    mx[node] = max(mx[lft] , mx[rgt]);
    mn[node] = min(mn[lft] , mn[rgt]);
}
void update(int node,int st,int en,int index,int new_val)   /// O(logN)
{
    ...
    (index<=md)?update(lft,st,md,index,new_val):update(rgt,md+1,en,index,new_val);


    mx[node] = max(mx[lft] , mx[rgt]);
    mn[node] = min(mn[lft] , mn[rgt]);
}
int query_mx(int node,int st,int en,int i,int j)   /// O(logN)
{
    if(i>en || j<st)
        return 0;
    if(st>=i && en<=j)
    ...
    return max(query_mx(lft,st,md,i,j) , query_mx(rgt,md+1,en,i,j));
}
int query_mn(int node,int st,int en,int i,int j)   /// O(logN)
{
    if(i>en || j<st)
        return MAX;
    if(st>=i && en<=j)
    ...
    return min(query_mn(lft,st,md,i,j) , query_mn(rgt,md+1,en,i,j));
}
...
int main()
{
    ...
    while(q--)
    {
        ...
        else
        {
            ...
            if(i>j)
                swap(i,j);
            out(query_mx(1,1,n,i,j)-query_mn(1,1,n,i,j));
            e;
        }
    }
