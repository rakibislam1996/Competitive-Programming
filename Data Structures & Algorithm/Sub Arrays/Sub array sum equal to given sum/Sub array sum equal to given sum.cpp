...
void subArraySum(ll arr[], ll n, ll sum)
{
	ll ans=0;
	...
	for (int i = 0; i < n; i++)
	{
		...
		{
			cout << "Sum found between indexes "<< m[curr_sum - sum] + 1<< " to " << i << endl;
			return;
		}
		else
            m[curr_sum] = i;    /// Here i is assigned only to show indices of array having sum = sum
                                /// otherwise m[curr_sum]++ will be enough
	}
