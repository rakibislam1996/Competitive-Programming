...
int main()
{
    ...
    TC(T)
    {
        ...
        ll hor = k<n-k ? k : n-k;
        ll lob = k>n-k ? k+1 : n-k+1;
        ll j;
        mem(p,0);
        for(ll t=lob,m=1; t<=n; t++,m++)
        {
            j = t;
            for(ll s=0; prime[s]*prime[s]<=j; s++)
                while(!(j%prime[s]))
                {
                    p[prime[s]]++;
                    j/=prime[s];
            ...
            if(m==1)
                continue;
            j = m;
            for(int s=0; prime[s]*prime[s]<=j; s++)
                while(!(j%prime[s]))
                {
                    p[prime[s]]--;
                    j/=prime[s];
        ...
        for(y=0; prime[y]<=n; y++)
            if(p[prime[y]]>0)
                ans = (ans%mod * bigMod(prime[y],p[prime[y]],mod))%mod;
        out(ans),e;
    }
    return 0;
}
