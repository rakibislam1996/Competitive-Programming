...
vi G[MX],cost[MX];
ll n,edge,k,dis[25][MX]; /// kth best path
void dij(int src)
{
    priority_queue<pii,vector<pii>,greater<pii>>pq;
    int i,j;
    For(i,1,k) For(j,1,n) dis[i][j]=inf;
    pq.push(pii(0,src));
    dis[1][src]=0;
    while(!pq.empty())
    {
        int u = pq.top().S, cst = pq.top().F; pq.pop();
        //cout<<"u = "<<u<<" cst = "<<cst<<E<<E;
        ...
        for(int i=0; i<G[u].size(); i++)
        {
          ...
          //  cout<<"v = "<<v<<" w = "<<w<<E;
            for(int j=1;j<=k;j++)
            {
            //    cout<<"j = "<<j<<" cst+w = "<<cst+w<<" dis = "<<dis[j][v]<<E;
                if(cst+w < dis[j][v])
                {
              //      cout<<"ekahne\n";
                    for(int p=k-1;p>=j;p--)
                    {
                //        cout<<p+1<<" = "<<p<<" -> "<<dis[p][v]<<E;
                        dis[p+1][v] = dis[p][v];
                    }
                    dis[j][v] = cst+w;
                    //cout<<j<<","<<v<<" = push "<<cst+w<<E;
                    pq.push(pii(dis[j][v],v));
                    break;
                }
                else if(cst+w == dis[j][v])
                    break;
            }
        }
    }
