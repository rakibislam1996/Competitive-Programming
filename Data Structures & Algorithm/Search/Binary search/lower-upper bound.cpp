    ...
    while(bg<=en)
    {
        ...
        if(arr[mid]==val)
        {
            ...
            while(upperBound<n && arr[upperBound]==val)
                upperBound++;
            upperBound--;
            out(lowerBound);space;out(upperBound);e;
            return;
        }
        else if(arr[mid]<val)
            bg = mid+1;
...
{
    int bg = 0,en=n-1,mid,c=0,lowerBound = -1,upperBound=-1;
    while(bg<=en)
    {
        mid = bg + ((en-bg)>>1);
        if(arr[mid]==val)
...
int main()
{
    ...
    FTC(T)
    {
        ...
        out(binarySearch(arr,n,val));
        e;
        out(bSearch(arr,n,val));
        e;
