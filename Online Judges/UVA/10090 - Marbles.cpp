...
int main()
{
    ...
    while((cin >> n) && n)
    {
        ...
        lli lowbound = (lli)ceil(-(double)x*n/n2);
        lli upbound = (lli)floor((double)y*n/n1);
        ...
        lli cost1 = (c1*(n2/g) - c2*(n1/g)) * lowbound;
        lli cost2 = (c1*(n2/g) - c2*(n1/g)) * upbound;
        if(cost1<cost2)
        {
            x = (x*n + n2*lowbound)/g;
            y = (y*n - n1*lowbound)/g;
