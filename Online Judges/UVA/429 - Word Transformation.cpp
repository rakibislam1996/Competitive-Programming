...
int bfs(vector<string> word,string source,string des)
{
    int i,j;
    string u,v;
    queue<string>q;
    ...
    q.push(source);
    level[source]=0;
    while(!q.empty() && level.count(des)==0)
    {
        u = q.front(); q.pop();
        for(string s : word)
        {
            v = s;
            if(level.count(v)==0 && u.size()==v.size())
            {
                int diff=0;
                for(i=0;i<u.size();i++)
                    if(u[i]!=v[i])
...
int main()
{
    ...
    FTC(T)
    {
        ...
        while(getline(cin,str) && str!="")
        {
            ...
            cout<<src<<" "<<des<<" "<<bfs(word,src,des)<<E;
        }
        if(cs!=T)
            cout<<E;
