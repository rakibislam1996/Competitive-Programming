...
int main()
{
    ...
    TC(F)
    {
        ...
        ld klog10n = k * ((log10(n)) * 1.0);

        int int_klog10n = int(klog10n);
        ld float_klog10n = klog10n - int_klog10n;

        //ulli nToK =  pow(10,int_klog10n) * pow(10,float_klog10n);
        ulli leading = (pow(10,float_klog10n) * 1000.0);
        string s = ItoS(leading);
        int len = s.length();
        cout<<s[0]<<s[1]<<s[2]<<"...";

        int trailing =  bigMod(n,k,1000ULL);
        if(trailing >= 100)
            cout<<trailing<<E;
        else if(trailing<10)
            cout<<"00"<<trailing<<E;
