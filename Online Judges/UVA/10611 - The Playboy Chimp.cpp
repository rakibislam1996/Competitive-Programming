...
void binarySearch(int arr[],int n,int val,int t)
{
    int bg=0,en=n-1,mid;
    while(bg<=en)
    {
        ...
        if(arr[mid]==val)
        {
            if(t==1)
            {
                int upperBound = mid;
                upperBound++;
                while(upperBound<n && arr[upperBound]==val)
                    upperBound++;
                out(arr[upperBound]);e;
                return;
            }
            else if(t==2)
            {
                int lowerBound = mid;
                while(lowerBound>=0 && arr[lowerBound]==val)
                    lowerBound--;
                out(arr[lowerBound]);
                puts(" X");
                return;
            ...
            else
            {
                ...
                while(upperBound<n && arr[upperBound]==val)
                    upperBound++;
                out(arr[lowerBound]);space;out(arr[upperBound]);e;
                return;
            }
        }
        else if(arr[mid]<val)
    ...
    if(arr[mid]<val)
    {
        out(arr[mid]);space;out(arr[mid+1]);e;
    }
    else if(arr[mid]>val)
    {
        out(arr[mid-1]);space;out(arr[mid]);e;
    }
    return;
}
int main()
                ...
                else
                {
                    ...
                    else if(arr[n-1]<=x)
                    {
                        if(arr[n-1]<x)
                            ...
                            binarySearch(arr,n,x,2);
                    }
                    else
                        binarySearch(arr,n,x,3);
                }
            }
        }
