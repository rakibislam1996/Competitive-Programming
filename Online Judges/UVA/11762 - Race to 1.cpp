...
void solve()
{
    ...
    For(i,2,MX)
    {
        ...
        dp[i] = (dp[i]+(upper_bound(all(prime),i)-prime.begin()))/k;
    }
}
int main()
{
...
/*
E(N) where N is a number,k is the number of it’s distinct prime factors and
p is the number of primes <= n
E(N)=1+(1/p)*(E(N/a1) + E(N/a2) + E(N/a3) … + E(N/ak)) + ((p-k)/p) * E(N)
it means after one step we can choose a correct prime numbers(from the factors of N)
or one of the wrong ones from (p-k) ones
...
E(N) = (p/k)  + (p/k) * (1/p)*(E(N/a1) + E(N/a2) + E(N/a3) … + E(N/ak))
E(N) = (p/k)  + (1/k) *(E(N/a1) + E(N/a2) + E(N/a3) … + E(N/ak))
E(N) = (1/k) * (p + (E(N/a1) + E(N/a2) + E(N/a3) … + E(N/ak)))
