...
ui mark[1000007];
vi segPrime;
//template<typename T>inline void segSieve(T a,T b)
//{
//    mem(mark,0);
//    if(a==1)
//        a++;
//    for(T i=1;prime[i]*prime[i]<=b;i++)
//    {
//        T p = prime[i];
//        T j = p*p;
//        if(j<a)
...
template<typename T>inline void segmentedSieve ( T a, T b ) {
    if(a==1) a++;

    mem(mark,0);

    for(T i = 0; i<prime.size() && prime[i]* prime[i]<= b; i++ ) {
        T p = prime[i];
        T j = p * p;
