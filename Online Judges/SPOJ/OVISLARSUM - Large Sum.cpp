...
int main()
{
    ...
    ll inv2 = inverseMod(2LL,mod);

    ll firsthalf = ((((m%mod)*((m-1)%mod))%mod) * (inv2%mod))%mod;
    firsthalf = ((firsthalf%mod) * (interval%mod))%mod;

    ll secondhalf = ((((extra%mod)*(extra+1)%mod)%mod) * (inv2%mod))%mod;

    ll sum = ((firsthalf%mod) + (secondhalf%mod))%mod;

    l--;
    interval = l/m;
    extra = l%m;

    firsthalf = ((((m%mod)*((m-1)%mod))%mod) * (inv2%mod))%mod;
    firsthalf = ((firsthalf%mod) * (interval%mod))%mod;

    secondhalf = ((((extra%mod)*(extra+1)%mod)%mod) * (inv2%mod))%mod;

    ll sum2 = ((firsthalf%mod) + (secondhalf%mod))%mod;

    ll ans;
    if(sum>sum2)
        ans = (sum%mod + negMod(-sum2,mod)%mod)%mod;
    else
        ans = sum - sum2;
