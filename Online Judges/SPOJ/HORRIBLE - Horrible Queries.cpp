...
void pushdown(ll node,ll st,ll en)
{
    if(!prop[node])
        return;
    ...
    if(st!=en)
    {
        ...
        tree[lft] += (md-st+1)*prop[node];
        prop[lft] += prop[node];

        tree[rgt] += (en-md)*prop[node];
        prop[rgt] += prop[node];
    }
    prop[node] = 0;
}

void update(ll node,ll st,ll en,ll i,ll j,ll val)
{
    pushdown(node,st,en);
    if(i>en || j<st)
        return;
...
ll query(ll node,ll st,ll en,ll i,ll j)
{
    pushdown(node,st,en);
