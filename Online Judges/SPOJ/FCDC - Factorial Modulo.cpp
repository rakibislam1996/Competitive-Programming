...
template<typename T>inline void sieve(T N){T i,sq=sqrt(N)+2;status[0]=status[1]=1;prime.pb(2);for(i=4;i<=N;i+=2)status[i]=1;for(i=3;i<=N;i+=2)if(!(status[i])){prime.pb(i);if(i<=sq)for(lli j=i*i;j<=N;j+=(i<<1))status[j]=1;}}
...
bool isDivisible(lli n)
{
    lli i,j,k,sz = m.size();
    for(i=sz-1;i>=0;i--)
    {
        lli temp = noOfprime(n,m[i].F);
