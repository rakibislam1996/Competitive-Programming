...
#define sz 100005
int arr[sz];
ll tree[sz<<2][3];  /// [][0] for %3=zero , [][1] for %3=one, [][2] for %3=two
ll prop[sz<<2];
...
void pushdown(int node,int st,int en)
{
    ...
    {
        prop[node]%=3;
        for(int i=1;i<=prop[node];i++)
        {
            swap(tree[node][0],tree[node][2]);  /// zero = two
            swap(tree[node][1],tree[node][2]);  /// one = zero , two = one
        }
...
void update(int node,int st,int en,int i,int j)
{
    ...
    update(rgt,md+1,en,i,j);

    tree[node][0] = tree[lft][0] + tree[rgt][0];
    tree[node][1] = tree[lft][1] + tree[rgt][1];
    tree[node][2] = tree[lft][2] + tree[rgt][2];
}
