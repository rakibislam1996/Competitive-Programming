...
vll prime,pFactor;
#define chkp(n)         (!(status[n>>6]&(1<<((n>>1)&31))))
#define setp(n)         (status[n>>6]|=(1<<((n>>1)&31)))
int status[10000007];
...
template<typename T>inline void bitsieve(T N){T i,j,sq=sqrt(N)+2;prime.pb(2);for(i=3;i<=N;i+=2)if(chkp(i)){prime.pb(i);if(i<=sq)for(j=i*i;j<=N;j+=(i<<1))setp(j);}}
template<typename T>inline bool isPrime(T n){if(n==2)return 1;if(n<=1||!(n&1))return 0;chkp(n)?1:0;}
...
template<typename T>inline ull SOD(T N)
{
    ull sod = 1,sz = prime.size();
    for(long i=0; i<sz &&  prime[i]*prime[i]<=N; i++)
    {
        T temp=prime[i];
        ull sum=1,term=1;
        while(!(N%temp))
