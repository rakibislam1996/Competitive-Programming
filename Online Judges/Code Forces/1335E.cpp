...
int main()
{
    ...
    TC(T)
    {
        ll n,i,j,k,l,lw=0,hi=0,bg=0,en=0,ans=0,tot1=0,tot2=0;
        in(n);
        ll a[n+1];
        vll v[30];
        For(i,1,n)
        ...
        for(i=1;i<=26;i++)
        {
            ...
            {
                bg=j,en=j,lw=0,hi=0,tot1=0,tot2=0;
                for(l=j;l<v[i].size();l++)
                {
                    bg = j,en = l;
                    tot1 = (en-bg+1);
                    for(k=1;k<=26;k++)
                    {
                        if(k==i || v[k].size()==0)
                            continue;
                        lw = lower_bound(v[k].begin(),v[k].end(),v[i][bg]) - v[k].begin();
                        hi = lower_bound(v[k].begin(),v[k].end(),v[i][en]) - v[k].begin();
                        hi = v[k].size()-hi;
                        tot2 = tot1 + (min(lw,hi)*2);
                        ans = max(ans,tot2);
                    }
                    ans = max(ans,tot1);
                }
