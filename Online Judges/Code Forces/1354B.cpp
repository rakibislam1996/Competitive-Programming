...
int main()
{
    ...
    FTC(T)
    {
        ...
        vll v;
        ll ans=2000000,en,st;
        For(i,0,a.size()-1)
        {
            v.pb(a[i]);

            st = en = lower_bound(all(b),a[i])-b.begin();
            if(en==b.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(a[i]-b[st]<b[en]-a[i])
                v.pb(b[st]);
            else
                v.pb(b[en]);

            st = en = lower_bound(all(c),a[i])-c.begin();
            if(en==c.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(a[i]-c[st]<c[en]-a[i])
                v.pb(c[st]);
            else
                v.pb(c[en]);
        ...
        For(i,0,b.size()-1)
        {
            ...
            st = en = lower_bound(all(a),b[i])-a.begin();
            if(en==a.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(b[i]-a[st]<a[en]-b[i])
                v.pb(a[st]);
            else
                v.pb(a[en]);

            st = en = lower_bound(all(c),b[i])-c.begin();
            if(en==c.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(b[i]-c[st]<c[en]-b[i])
                v.pb(c[st]);
            else
                v.pb(c[en]);
        ...
        For(i,0,c.size()-1)
        {
            ...
            st = en = lower_bound(all(b),c[i])-b.begin();
            if(en==b.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(c[i]-b[st]<b[en]-c[i])
                v.pb(b[st]);
            else
                v.pb(b[en]);

            st = en = lower_bound(all(a),c[i])-a.begin();
            if(en==a.size())
                en--,st--;
            if(en!=0)
                st = en-1;
            if(c[i]-a[st]<a[en]-c[i])
                v.pb(a[st]);
            else
                v.pb(a[en]);
