...
void set1(ll indx,ll val)
{
    ...
    set1(indx+1,val);
    set1(indx+1,val+coin[indx]);
    set1(indx+1,val+2*coin[indx]);
}

void set2(ll indx,ll val)
{
    if(val>amount)
        return;
    if(indx==n)
    ...
    set2(indx+1,val);
    set2(indx+1,val+coin[indx]);
    set2(indx+1,val+2*coin[indx]);
...
int main()
{
    ...
    TC(T)
    {
        ...
        For(i,0,sz-1)
        {
            if(binary_search(all(v1),amount-v2[i]))
            {
                ans = 1;
                break;
            }
