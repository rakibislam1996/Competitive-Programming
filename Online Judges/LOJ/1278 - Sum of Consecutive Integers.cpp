...
2N is multiple of n, so n can be any divisor of 2N except 1
We have find how many valid n & a exist
here :  for n and (2a+n-1)
1. 2a+n-1 > n
2. one is even and another is odd
we have to find number of odd divisors of 2N ,
since there will be a even divisor for each odd divisor
to find number of even divisors with just have to skip exponent of prime divisor 2
from the following formula;

Now N  = p1^e1 * p2^e2 * .. *pl^ek
so ans is = (e2+1) *..* (ek+1)
since 1 is also a odd divisor, and n>1 so final ans is = ((e2+1) *..* (ek+1) - 1)
*/
