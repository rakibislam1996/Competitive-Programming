...
ll maxn = 1000000000000,indx,newIndx;
ll arr[MX+10];
void luckyNum(ll sum)
{
    if(sum>0 && sum<=maxn)
        arr[indx++]=sum;
    if(sum>maxn)
...
void veryLucky(ll sum,ll st)
{
    if(sum<=0 || sum>maxn)
        return;
    if(sum>1)
        arr[newIndx++] = sum;
    for(ll i=st;i<indx;i++)
    {
        ll k = arr[i]*sum;
        //cout<<" st = "<<st<<" i = "<<i<<" k = "<<k<<" sum = "<<sum<<" arr[i] = "<<arr[i]<<E;
        if(k>maxn || k<=0)
        {
            //cout<<"break\n";
            break;
...
int main()
{
    ...
    veryLucky(1,0);
    //cout<<newIndx<<E;
    sort(arr,arr+newIndx);
    indx = unique(arr,arr+newIndx)-arr;
    //cout<<indx<<E;
    TC(T)
    {
        ll a,b;
        ...
        ll lw = lower_bound(arr,arr+indx,a)-arr;
        ll hi = upper_bound(arr,arr+indx,b)-arr;
