...
int main()
{
    ...
    FTC(T)
    {
        sc("%lld%lld%lld%lld%lld%lld%lld",&A, &B, &c, &x1, &x2, &y1, &y2);
        lli soln;
        c = -c;
        if(!A && !B)
        {
            soln = (c) ? 0 : (x2-x1+1)*(y2-y1+1);
        }
        else if(!A)
        {
            soln = (c%B) ? 0 : (((c/B)>=y1 && (c/B)<=y2)? (x2-x1+1) : 0);
        }
        else if(!B)
        {
            soln = (c%A) ? 0 : (((c/A)>=x1 && (c/A)<=x2)? (y2-y1+1) : 0);
        }
        else
        {
            if(A<0)
            ...
            {
                lli lowbound = max((lli)ceil((double)(x1*g-x*c)/B),(lli)ceil((double)(y*c-y2*g)/A));
                lli upbound = min((lli)floor((double)(y*c-y1*g)/A),(lli)floor((double)(x2*g-x*c)/B));
                soln = max(0LL,upbound-lowbound+1);
            }
