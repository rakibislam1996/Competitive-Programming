...
{
    int u,cost,d;
    node(int _u,int _cost,int _d){
        u = _u, cost = _cost, d = _d;
    }
    bool operator > (const node &x) const {
        return cost > x.cost;
    }
};
vi G[MX],cost[MX],xG[MX],xcost[MX];
int n,m,k,d,dis[12][MX];

void dij(int src)
{
    int i,j;
    priority_queue<node,vector<node>,greater<node>>pq;
    For(i,0,10) For(j,0,n) dis[i][j]=inf;
    pq.push(node(src,0,0));
    dis[src][0]=0;
    while(!pq.empty())
    {
        node x = pq.top(); pq.pop();
        int u = x.u, cst = x.cost, dt = x.d;
        for(i=0; i<G[u].size(); i++)
        {
            ...
            {
                dis[dt][v] = cst + w;
                pq.push(node(v,dis[dt][v],dt));
            }
        }
        for(i=0; i<xG[u].size(); i++)
        {
            int v = xG[u][i], w = xcost[u][i];
            if(cst+w < dis[dt+1][v])
            {
                dis[dt+1][v] = cst + w;
                pq.push(node(v,dis[dt+1][v],dt+1));
            }
        }
    }
}
...
int main()
{
    ...
    {
        int i,j,u,v,w;
        For(i,0,n) G[i].clear(),cost[i].clear(),xG[i].clear(),xcost[i].clear();
        in2(n,m);in2(k,d);
        For(i,1,m)
        {
            in3(u,v,w);
        ...
        {
            in3(u,v,w);
            xG[u].pb(v); xcost[u].pb(w);
        }
        dij(0);

        int ans = inf;
        For(i,0,d) ans = min(ans,dis[i][n-1]);
        cspf;
