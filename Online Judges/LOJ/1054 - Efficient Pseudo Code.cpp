...
ll SOD(ll N,ll M)
{
    ll ans=1,sz = prime.size();

    for(ll i=0; i<sz && prime[i]*prime[i]<=N; i++)
    {
        ll temp = prime[i],count=0;
        if(!(N%temp))
        {
            while(!(N%temp))
            {
            ...
            count = (M*count)+1;
            ll lob = (bigMod(temp,count,MOD)-1)%MOD;
            ll hor = bigMod(temp-1,MOD-2,MOD);
            ll s = (( lob%MOD) * (hor%MOD )) % MOD;
