...
int main()
{
    ...
    FTC(T)
    {
        ...
        for(int i = 0; i <= n; )
        {
            int W = ans + sum[i];
            cnt++;
            int pos = upper_bound(sum+i, sum+n+2, W) - sum - 1;
            int have = n + 1 - pos;
            int need = k + 1 - cnt;
            //cout<<"pos = "<<pos<<" cnt = "<<cnt<<" have = "<<have<<" need = "<<need<<E;
            if(have < need)
            {
                pos = n + 1 - need;
            }
            //cout<<"sum[pos] = "<<sum[pos]<<" sum[i] = "<<sum[i]<<E;
            out(sum[pos] - sum[i]);
            e;

            i = pos;
            //cout<<"i and pos = "<<pos<<E;
        }
    }
