...
template<typename T> T in3(T &a,T &b,T &c){in(a);in(b);in(c);}
template<typename T> void out(T x){char a[21];T i=1,j;if(x<0){putchar('-');x=~(x-1);}if(x==0)putchar('0');while(x){a[i++]=x%10+48;x/=10;}for(j=i-1;j>=1;j--)putchar(a[j]);}
...
bool flag = true;
template<typename T> T division(string str,T m)
{
    string d = str;
    T remainder=0;
    //for(int i=0; str[i];i++)
...
int main()
{
    ...
    FTC(T)
    {
        ...
        //string str = d;
        cout<<"Case "<<i<<": "<<division(d,n)<<E;

        //cout<<str<<endl;
        //ulli remainder = 0
        //remainder = (remainder*10 + str[i]-'0')%n;
        //while()
    }
