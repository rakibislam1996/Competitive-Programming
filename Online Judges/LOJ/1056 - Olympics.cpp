...
int main()
{
    ...
    TC(T)
    {
        char str[100000],st[10];
        scl(str);
        db l,w,len,wid;
        sscanf(str,"%lf%s%lf",&l,st,&w);
        cspf;

        /*
        db r = sqrt(l*l+w*w)/2.0;
        db s = r * acos(1-((w*w)/(2*r*r)));
        db factor = 200.0/ (l+s);
        pf("%.8f %.8f\n",l*factor,w*factor);
        */


        db lw = 0,hi=400,md,r,x=200;
        while(fabs(hi-lw)>eps)
        {
            md = lw + (hi-lw)/2.0;
            len = l*md;
            wid = w*md;
            r = sqrt(len*len+wid*wid)/2.0;
            db para = 2.0*(len) + ( sqrt(len*len+wid*wid) * acos(1-((wid*wid)/(2*r*r))) );
            if(para+eps<=400)
                lw = md;
            else
                hi = md;
