...
int main()
{
    ...
    TC(T)
    {
        ...
        in2(n,m);
        ll sum=0,ans=0,x,i;
        dp[0] = 1;  // 0 means,divisible by m,that remainder is 0,hence it is a subsequence divisible by m
        For(i,1,n)
        {
            in(x);
            sum = (sum + x)%m;
...
/*
if the two prefixes are the same as the remainder of m,
then the sum of the middle segments is a multiple of m.

Dp[i] indicates the number of times the prefix's remainder are i.
Because the substring can be combined with the previous one,
so add 2 when it appears 2 times, add 2 when it appears 3 times, and so on.
*/
