...
void update(int node,int st,int en,int i,int val)
{
    ...
    update(lft,st,md,i,val);
    update(rgt,md+1,en,i,val);
}
...
int query(int node,int st,int en,int i)
{
    ...
    (tree[lft]>=i)?query(lft,st,md,i):query(rgt,md+1,en,i-tree[lft]);
}
    ...
    FTC(T)
    {
        ...
        while(q--)
        {
            ...
            else
            {
                ...
                {
                    m--;
                    int index = query(1,1,nq,k);
                    //cout << "index = " << index << E;
                    out(arr[index]);e;
                    update(1,1,nq,index,-1);
                }
            }
        }
    }
