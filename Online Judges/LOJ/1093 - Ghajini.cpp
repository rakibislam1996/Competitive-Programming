...
void update(int node,int st,int en,int index,int new_val)
{
...
int query_mx(int node,int st,int en,int i,int j)
{
    if(i>en || j<st)
        return 0;
    if(st>=i && en<=j)
...
int query_mn(int node,int st,int en,int i,int j)
{
    if(i>en || j<st)
        return MAX;
    if(st>=i && en<=j)
...
int main()
{
    ...
    FTC(T)
    {
        ...
        while(r+1<=n)
        {
            int dif = query_mx(1,1,n,l,r)-query_mn(1,1,n,l,r);
            if(mx<dif)
                mx = dif;
            l++;r++;
        }
