...
Sum += 3+4+5
Sum += (5-3)*3
Then, Another divisors are 4. It starts from N = 16.  Notice it starts from divisor '4' and ends at 16/4 = 4.
There is no remaining divisors '4' below.
Sum += 4
Sum += (4-4)*4
Lets calculate sum all of them systematically:
For divisor i = 2, j = 16/2 = 8
Sum = i + i+1 + i+2 + ... + j
    = (1+2+3+ ... +j) - (1+2+3+(i-1))
    = (j*(j+1))/2 - ((i-1)(i))/2
    = (j-i)(j-i+1)/2   <- Skipped few steps
Straight line sum, for divisor i = 2, j = 16/2 = 8
Sum = (j-i)*i
So value of i will be in range [2...sqrt(n)]
*/
