...
void mid1(double x1,double y1,double z1,double x2,double y2,double z2)
{
    a1 = x1+ ((x2/3.0)-(x1/3.0));
    b1 = y1+ ((y2/3.0)-(y1/3.0));
    c1 = z1+ ((z2/3.0)-(z1/3.0));
}
void mid2(double x1,double y1,double z1,double x2,double y2,double z2)
{
    a2 = a1+ ((x2/3.0)-(x1/3.0));
    b2 = b1+ ((y2/3.0)-(y1/3.0));
    c2 = c1+ ((z2/3.0)-(z1/3.0));
...
int main()
{
    ...
    //std::ios_base::sync_with_stdio(false);
        ...
        FTC(T)
        {
            ...
            sc("%lf%lf%lf%lf%lf%lf%lf%lf%lf",&x1, &y1, &z1, &x2, &y2, &z2, &x, &y, &z);
            int c = 0;
            double ans = ULONG_MAX,dis1,dis2;

            while(c<=100)
            {
                c++;
                mid1(x1, y1, z1, x2, y2, z2);
                mid2(x1, y1, z1, x2, y2, z2);

                dis1 = sqrt(sqr(a1-x)+sqr(b1-y)+sqr(c1-z));
                dis2 = sqrt(sqr(a2-x)+sqr(b2-y)+sqr(c2-z));
