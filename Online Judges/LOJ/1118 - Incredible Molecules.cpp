...
int main()
{
    ...
    FTC(T)
    {
        ...
        double r1r2 = sqrt(sqr(x1-x2)+sqr(y1-y2));
        cspf;

        if(r1+r2<=r1r2)
            puts("0");
        else if(fabs(r1-r2)<r1r2)
        {
            double theta = acos((r1*r1 + r1r2*r1r2 - r2*r2)/(2.0*r1*r1r2));
            theta *= 2.0;
            double tri = (r1*r1*sin(theta))/2.0;
            double fan = (r1*r1*theta)/2.0;
            double area1 = fan - tri;

            theta = acos((r2*r2 + r1r2*r1r2 - r1*r1)/(2.0*r2*r1r2));
            theta *= 2.0;
            tri = (r2*r2*sin(theta))/2.0;
            fan = (r2*r2*theta)/2.0;
            double area2 = fan  - tri;
...
area of triangle = (a*b*sin(C))/2.0

area of fan shaped region : (r^2*theta)/2.0;
