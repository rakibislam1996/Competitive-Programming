...
int main()
{
    ...
    FTC(T)
    {
        ...
        ll ans = (sum * (k%mod) * bigMod(n,k-1,mod))%mod;
        cspf;
        out(ans),e;
    }
...
There are k-layer loops in the code, and each layer is looped n times,
that is, n^k additions are performed.
Each addition can select k numbers from n numbers,
and the probability that each number is selected is k/n.
So a number is added (k/n * n^k) = k*n^(k-1) times.
