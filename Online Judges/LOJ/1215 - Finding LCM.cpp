...
/*
GCD(a,b) is the smaller index of a,b's common prime factor in a,b

LCM(a,b) is the larger index of all prime factors of a and b taken in a and b.

Let m = LCM(a,b) then the problem translates to the minimum c satisfying LCM(m, c) = L

Then the smallest c is the prime factor in L that is not in m
and the exponent in L where the exponent is greater than the exponent in m.

If c and m are mutually prime, then c will be the smallest,
so we can assume gcd(c , m ) == 1. If we can reach this state, then c is ans.
The required value, if this state is not reached,
increase c, and decrease m
this time c = c * gcd ( c , m )  , m = m / gcd ( c , m ),
until Find the value of c when gcd ( c , m ) == 1 .
*/
