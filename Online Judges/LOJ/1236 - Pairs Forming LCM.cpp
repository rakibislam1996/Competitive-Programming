...
template<typename T>inline T solve(T N)
{
...
Therefore, when lcm(a,b)==n, max(a1,b1)==e1,max(a2,b2)==e2,...max(ak,bk)==ek;

When ai == ei and bi != ei then bi can take all the numbers in [0, ei-1] with ei cases,
and bi == ei and ai != ei the same.

Then there are 2ei cases total, ans there is additional case when ai==bi,so
total case = 2ei+1 for each pi

Now, Except for (n, n), all cases occur twice, so
ans = i(p1 to pk)summation (2ei+1)
ans = (ans / 2) + 1.
*/
