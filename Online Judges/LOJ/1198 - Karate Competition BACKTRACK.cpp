...
ll solve(ll i,ll j)
{
    ...
    {
        c = 1 + solve(i+1,j+1);
        ll idx = upper_bound(a,a+n,b[j])-a;
        if(idx<=n-1)
            c = max( c , (2+solve(idx+1,j+1)) );
    }
...
/*
if a[i] > b[j] then Return 2 + solve(i+1,j+1)
if a[i] < b[j] then Return solve(i+1,j)
if a[i] == b[j] :
    1. To draw :  c = 1 + solve(i+1,j+1)
    2. To find a person in my team,a[i] with skill greater than this b[j].
        idx = lowest index of the person with skill greater than b[j]
        if(idx <= n-1)
             Return max(c , 2 + solve(idx + 1,j + 1))
*/
