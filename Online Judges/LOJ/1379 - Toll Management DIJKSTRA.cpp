...
vi G[MX],cost[MX],G2[MX],cost2[MX];
...
int main()
{
    ...
    TC(T)
    {
        ...
        in2(node,edge),in3(frm,to,p);
        while(edge--)
        {
            int u,v,w;
            in3(u,v,w);
            G[u].pb(v); cost[u].pb(w);
            G2[v].pb(u); cost2[v].pb(w);
        }
        dij(frm); dij2(to);
        int mx = -1;
        for(int u = 1; u<= node; u++)
        {
            ...
            G[u].clear(); cost[u].clear();
            G2[u].clear(); cost2[u].clear();
        }
        if(mx == inf) mx = -1;
        cspf,out(mx),e;
    }
