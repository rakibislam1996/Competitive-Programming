...
template<typename T>inline void sieve(T N){int i,j,sq=sqrt(N)+2;status[0]=status[1]=1;prime.pb(2);for(i=4;i<=N;i+=2)status[i]=1;for(i=3;i<=N;i+=2)if(!(status[i])){prime.pb(i);if(i<=sq)for(j=i*i;j<=N;j+=(i<<1))status[j]=1;}}
template<typename T>inline void bitsieve(T N){int status[(N>>5)+2];memset(status,0,((N>>5)+2)<<2);for(T i=3;i*i<=N;i+=2)if(!(status[i>>5]&(1<<(i&31))))for(T j=i*i;j<=N;j+=(i<<1))status[j>>5]|=(1<<(j&31));prime.pb(2);for(T i=3;i<=N;i+=2){if(!(status[i>>5]&(1<<(i&31))))prime.pb(i);}}
...
template<typename T>inline ulli power(T A,T N){int lengthOfN=log2(N);ulli result=1;while(lengthOfN>=0){result*=result;if(N&(1<<lengthOfN))result*=A;lengthOfN--;}return result;}
...
template<typename T>inline T division(string str,T m){string d=str;T remainder=0;forstr(str){remainder=(remainder*10+(str[i]-'0'))%m;if(!remainder){return str.length();}else str+=d;}}
template<typename T>inline T BtoD(T x){T two=1,intn=0;while(x){if(x%10)intn+=two;x/=10;two<<=1;}return intn;}
inline ulli BtoD(string str){ulli intS=0;int len=str.length();for(int i=0;str[i];i++)intS=(intS<<1)+(str[i]-'0');return intS;}
...
int main()
{
    ...
    FTC(T)
    {
        ...
        lli ans = (f[k-1]%m * f[n]%m)%m;

        ans = (f[n+k-1]%m * bigMod(ans,m-2,m)%m)%m;
        Cspf;
        out(ans);
        e;
    }
