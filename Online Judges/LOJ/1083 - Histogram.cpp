...
ll query(int node,int st,int en,int i,int j)
{
    ...
    ll x = query(lft,st,md,i,j);
    ll y = query(rgt,md+1,en,i,j);
...
{
    if(st<1 || en<1 || st>n || en>n || st>en)
        return 0;
    if(st==en)
        return arr[st];

    ll md = query(1,1,n,st,en);
    ll area1 = arr[md]*(en-st+1);
    ll area2 = solve(st,md-1);
    ll area3 = solve(md+1,en);
