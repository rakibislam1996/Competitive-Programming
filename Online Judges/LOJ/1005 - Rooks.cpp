...
template<typename T>inline ull power(T A,T N){T len=log2(N);ull res=1;while(len>=0){res*=res;if(N&(1<<len))res*=A;len--;}return res;}
...
int main()
{
    ...
    FTC(T)
    {
        ...
        for(i=0;prime[i]<=n;i++)
        {
            m[prime[i]] = (noOfprime(n,prime[i]));
        }
        for(i=0;prime[i]<=(n-k);i++)
        {
            m[prime[i]] -= noOfprime(n-k,prime[i]);
        }
        for(i=0;prime[i]<=k;i++)
        {
            m[prime[i]] -= noOfprime(k,prime[i]);
        }
        forstl(i,m)
        {
            (*i).S <<= 1;
