...
ll coinChange()
{
    ...
    For(i,1,n)  // coin
    {
        ...
        {
//    cout<<"\ncoin = "<<coin[i]<<" j = "<<j<<E;
//    cout<<"dp["<<j<<"]="<<dp[j]<<" j-coin[i]="<<j<<"-"<<coin[i]<<" dp[j-coin[i]]="<<dp[j-coin[i]];
//    cout<<" used[j-coin[i]]="<<used[j-coin[i]]<<" limit[i]="<<limit[i]<<E;

            if(dp[j]==0 && dp[j-coin[i]] && used[j-coin[i]]<limit[i])
            {
//                cout<<"dhukse\n";
                dp[j] = 1;
                ans++;
...
/*
dp[i] indicates whether it can form i , true/false
used[i] means how many times coins[i] used to form i
*/
