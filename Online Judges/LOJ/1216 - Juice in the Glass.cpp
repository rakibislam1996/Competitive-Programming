...
int main()
{
    ...
    {
        double r1,r2,h,p;
        sc("%lf%lf%lf%lf",&r1,&r2,&h,&p);
        double x = (h*r2) / (r1-r2);
        double r = (p*(r1-r2))/h + r2;
        double vol = (PI/3) * ((r*r * (p+x)) - ((r2*r2)* x));
        //double vol = ((PI*p)/3) * (r*r + r*r2 + r2*r2);
        cspf;
        pf("%f\n",vol);
...
/*
considering the glass as cone by adding corner part to the lower part of glass,
let height of the lower part of cone is x
according to similar angular triangle property
r2/x = r/(p+x)
x = h*r2 / (r1-r2)

again, according to similar angular triangle property
r/(p+x) = r2/x
r = (p*(r1-r2))/h + r2

now , using volume of cone
ans = 1/3 * PI*r^2 (p+x) - 1/3*PI*r2^2 x
    = PI/3 *(r^2 * (p+x)  - r2^2 x)

and , using volume of frustum
ans =  (PI*p)/3 * (r^2 + r*r2 + r2^2)
*/
