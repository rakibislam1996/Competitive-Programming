...
//            if(distance(point[0],point[i]) == distance(point[0],p) + distance(p,point[i]))
//                return true;
//    }
//    return false;
//}

ll triArea(pll p1,pll p2,pll p3)
{
    return (p1.F*(p2.S-p3.S) + p2.F*(p3.S-p1.S) + p3.F*(p1.S-p2.S));
}
...
bool inside(pll p)
{
    ...
    while((hi-lw)>1)
    {
        ...
        if(triArea(point[0], point[md], p)<0)
            hi = md;
        else
            lw = md;
    }
    ll part1 = abs(triArea(point[lw], point[hi], p));
    ll part2 = abs(triArea(point[0], point[hi], p));
    ll part3 = abs(triArea(point[0], point[lw], p));
    return ((part1+part2+part3)==abs(triArea(point[0], point[lw], point[hi])));
}
