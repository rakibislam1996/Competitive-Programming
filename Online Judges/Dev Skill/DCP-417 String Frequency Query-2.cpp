...
void update(int node,int st,int en,int i,int sg,int val)
{
    if(i>en || i<st)
        return;
    if(st>=i && en<=i)
    ...
    update(lft,st,md,i,sg,val);
    update(rgt,md+1,en,i,sg,val);

    tree[node][sg] = tree[lft][sg] + tree[rgt][sg];
}

int query(int node,int st,int en,int i,int j,int sg)
{
    ...
    return query(lft,st,md,i,j,sg)+query(rgt,md+1,en,i,j,sg);
}
