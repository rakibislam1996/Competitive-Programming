...
void update(int node,int st,int en,int index,int new_val)   /// O(logN)
{
    ...
    update(lft,st,md,index,new_val);
    update(rgt,md+1,en,index,new_val);

    mx[node] = max(mx[lft] , mx[rgt]);
    //mx[node] = min(mx[lft] , mx[rgt]);

}
int query(int node,int st,int en,int i,int j)   /// O(logN)
{
    if(i>en || j<st)
        return 0;
        //return MAX;
    ...
    return max(query(lft,st,md,i,j) , query(rgt,md+1,en,i,j));
    //return min(query(lft,st,md,i,j) , query(rgt,md+1,en,i,j));
}
vector<ll>s[2*MX+10];
int main()
{
    ...
    TC(T)
    {
        ...
        while(q--)
        {
            ...
            j++;
            k++;
            ll m = query(1,1,n,j,k);
            ll l = upper_bound(s[m].begin(),s[m].end(),k) - s[m].begin() - 1;
            ll r = lower_bound(all(s[m]),j) - s[m].begin();
            out(l-r+1),e;
        }
